...
namespace Thalamus
{
    public class ThalamusServerService : XmlRpcListenerService, IThalamusServer
    {
        ...
        public string Connect(string name, string clientId, int port, int numSubscribedEvents)
        {
			return clients.ConnectClient(name, clientId, clientAddress, port, numSubscribedEvents);
        }
        ...
        public void Pong(string clientId)
        {
            if (clients.RemoteClients.ContainsKey(clientId))
            {
                clients.RemoteClients[clientId].Pong();
            }
        }

        public void AnnounceEventInformation(string clientId, string[] perceptionName, string[][] parameters, string[][] types, string[] enumNames, string[][] enums)
        {
            clients.AnnounceEventInformation(clientId, perceptionName, parameters, types, enumNames, enums);
        }

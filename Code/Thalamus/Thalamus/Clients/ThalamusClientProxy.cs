...
namespace Thalamus
{
    public class ThalamusClientProxy
    {
        ...
        public ThalamusClientProxy(ClientsInterface body, string name, string address, int port, string clientId, int numSubscribedEvents)
        {
            this.name = name;
            this.port = port;
            ...
            fastProxy.Url = string.Format("http://{0}:{1}", address, port);
            fastProxy.KeepAlive = true;
            fastProxy.Timeout = 500;

			/*slowProxy = XmlRpcProxyGen.Create<IThalamusClientRpc>();
			slowProxy.Url = string.Format("http://{0}:{1}", address, port);
        ...
        public override string ToString()
        {
            ...
            return string.Format("'{0}'@{1}:{2} {3}",name,address, port, (str==""?"":("{"+str+"}")));
        }

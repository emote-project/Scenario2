...
namespace BenchmarkClient
{
	...
	public class BenchmarkClient : ThalamusClient, IBenchmarkActions
	{
		...
		public BenchmarkClient (frmBenchmark form, int iterations, int fps, int numRounds, bool publish=true) : base("Benchmark" + BenchMarkCount++.ToString(), false)
		{
            this.form = form;
		...
		void IBenchmarkActions.MessageReceived (string clientName, int msgId, int ticks)
		{
            lock (gotMessages)
            {
                if (!gotMessages.ContainsKey(clientName)) gotMessages[clientName] = new int[iterations + 1];
                bool ok = false;
                int d = 0;
                int i = 0;
                while (!ok && i < 5)
                ...
                //Debug("Cli: {0}; Msg:{1}; delay: {2}", clientName, msgId, d);
        ...
        public void PrintBenchmarkStatistics()
        {
            ...
            foreach (KeyValuePair<string, int[]> msg in gotMessages)
            {
                for (int i = 1; i < iterations; i++)
                {
                    //if (msg.Value[i] != numRounds)

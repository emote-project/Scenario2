...
app.test_policy = app.output_path+"/simpleds-policy-13.json";
...
ws.onmessage = function(evt) {
	if(world == null) { 
		...
		alert("World is not started yet!");
		ws.close();
	} else {
		world.tick(evt.data);
		world.save();
...
World.prototype = {      
		tick: function(rawFeatures) {
			...
			this.agents[0].backward(this.observedActions, this.observedReward);
...
Agent.prototype = {
		...
		backward: function(observedActions, observedReward) { 
			var numreward = parseFloat(observedReward);
